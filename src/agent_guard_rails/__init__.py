...
class GuardRail:
    ...
    def add_length(self, min_chars: int | None = None,
                   max_chars: int | None = None) -> "GuardRail":
    ...
    def add_no_pattern(self, pattern: str, name: str = "no_pattern",
                       flags: int = re.IGNORECASE) -> "GuardRail":
        """Block output that matches a forbidden regex pattern."""
        compiled = re.compile(pattern, flags)

        def rule(output: Any) -> None:
            if compiled.search(str(output)):
                raise GuardrailViolation(name, f"Forbidden pattern matched: {pattern!r}", output)
        return self.add(name, rule)

    def add_required_pattern(self, pattern: str, name: str = "required_pattern",
                             flags: int = re.IGNORECASE) -> "GuardRail":
        ...
        def rule(output: Any) -> None:
            if not compiled.search(str(output)):
                raise GuardrailViolation(name, f"Required pattern missing: {pattern!r}", output)
        return self.add(name, rule)

    def add_no_keywords(self, keywords: list[str], name: str = "no_keywords",
                        case_sensitive: bool = False) -> "GuardRail":
        ...
        if not case_sensitive:
            ...
            def rule(output: Any) -> None:
                ...
                if found:
                    raise GuardrailViolation(name, f"Forbidden keywords: {found}", output)
        else:
    ...
    def run(self, output: Any) -> GuardrailResult:
        ...
        return GuardrailResult(passed=len(violations) == 0, output=output, violations=violations)
...
__all__ = ["GuardRail", "GuardrailViolation", "GuardrailResult"]
